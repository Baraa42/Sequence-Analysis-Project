...
test_dataset = dataset.Translation(
    max_len_input,
    max_len_target,
...
torch.save(
    model, os.path.join(translation_path, f"translation_attention_{NUM_SAMPLES}.pth")
)
...
while True:
    ...
    if len(translation) > 0:
        print(
            "BLEU Score", bleu.sentence_bleu([pred_translation], true_translation)
    ...
    for i in range(len(input_texts)):
        ...
        if len(translation) > 0:
            score += bleu.sentence_bleu(
                [pred_translation], true_translation
            )  # , smoothing_function=smoothie))
